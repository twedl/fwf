...
- **Columnar reader** (polars shape): split the input into chunks and parse them in parallel (rayon), then join the chunks in order into Arrow `RecordBatch`es. Within a chunk, parsing goes a record at a time, pushing each field into its column's builder, as polars does. Widths count characters, so a UTF-8 line's byte offsets are worked out once and shared by all its fields; a column-at-a-time loop would have to redo or store them for every line.
...
src/
  ...
  encoding.rs     Encoding { Utf8, Cp1252, Cp850 }: character positions → byte offsets, bytes → UTF-8
  ...
  field.rs        Spans: the schema's sorted field boundaries; one field's trimmed value
  builder.rs      column builders → Arrow arrays (String → Utf8, Float64 → Float64, all nullable)
  read_impl.rs    parse(unit, bytes) → RecordBatch; later the slice path (parallel) and stream path
...
| `read_impl.rs` | `read_impl.rs` | Same chunk-and-rayon shape, parsing a record at a time; chunk boundaries never need checking. |
...
// The encoding lives in ReadOptions: framing and decoding need it, opening inputs doesn't.
...
- Strip a UTF-8 BOM at the start of a UTF-8 unit; left in, it shifts every offset by three. In a cp1252 or cp850 unit those bytes are data.
...
All three encodings are ASCII-compatible, so framing, trimming, null checks and `Float64` parsing work on raw bytes. The encoding matters in two places: finding a field's byte range (UTF-8 only) and converting a `String` field to UTF-8. `Encoding` is a plain enum passed into the parsing loop; its per-field `match` always takes the same branch, so no generics are needed unless profiling says otherwise.
...
- `scripts/gen_tables.py` writes `src/tables.rs` from Python's codecs. Tests decode the cp1252 and cp850 fixtures to the UTF-8 one, and check sample bytes and the five undefined cp1252 bytes.
- A wrong code page fails silently: bytes `4A 6F 73 E9` read as `José` in CP1252 but `JosÚ` in CP850.
- Widths count characters. In cp1252 and cp850 that is the same as bytes. In UTF-8, `Encoding::byte_offsets` converts all of a line's field boundaries to byte offsets in one pass; pure-ASCII lines skip the scan.
